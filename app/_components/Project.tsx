...
const Project = ({ index, project, selectedProject, onMouseEnter }: Props) => {
    ...
    return (
        <a
            ...
            className="project-item group leading-none py-5 md:border-b first:!pt-0 last:pb-0 last:border-none md:group-hover/projects:opacity-30 md:hover:!opacity-100 block transition-all"
        ...
        >
            ...
            <div className="flex gap-2 md:gap-5 4k:gap-10 overflow-hidden">
                <div className="font-anton text-muted-foreground text-sm mobile-m:text-base 4k:text-6xl shrink-0">
                    _{(index + 1).toString().padStart(2, '0')}.
                </div>
                <div className="min-w-0">
                    <h4 className="text-3xl mobile-m:text-4xl xs:text-6xl laptop-l:text-8xl 4k:text-[180px] flex gap-2 mobile-m:gap-4 4k:gap-10 font-anton transition-all duration-700 bg-gradient-to-r from-primary to-foreground from-[50%] to-[50%] bg-[length:200%] bg-right bg-clip-text text-transparent group-hover:bg-left">
                        {project.title}
                        <span className="text-foreground opacity-0 group-hover:opacity-100 transition-all flex items-center">
                            <svg
                                ...
                                className="w-6 h-6 mobile-m:w-9 mobile-m:h-9 laptop-l:w-16 laptop-l:h-16 4k:w-32 4k:h-32"
                            >
                                <path
                                    id="box"
                    ...
                    <div className="mt-2 4k:mt-10 flex flex-wrap gap-2 mobile-m:gap-3 4k:gap-8 text-muted-foreground text-[10px] mobile-m:text-xs laptop-l:text-xl 4k:text-4xl">
                        {project.techStack
                            .slice(0, 3)
                            .map((tech, idx, stackArr) => (
                                <div
                                    className="gap-2 mobile-m:gap-3 4k:gap-6 flex items-center"
                                    key={tech}
                                >
                                    <span className="">{tech}</span>
                                    {idx !== stackArr.length - 1 && (
                                        <span className="inline-block size-1 mobile-m:size-2 4k:size-4 rounded-full bg-background-light"></span>
                                    )}
                                </div>
                            ))}
                    </div>
